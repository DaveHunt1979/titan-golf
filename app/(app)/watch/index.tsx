...
export default function WatchScreen() {
  ...
  const load = useCallback(async () => {
    ...
    if (allIds.length > 0) {
      const { data: pd } = await supabase.from('players').select('id,display_name').in('id', allIds);
      if (pd) setPlayers(pd);
    }
        ...
        >
          ...
          {live.length > 0 && (
            <>
              ...
              {live.map(m => (
                <MatchCard key={m.id} match={m} firstName={firstName} onWatch={() => router.push(`/(app)/spectate/${m.id}` as any)} />
              ))}
            </>
          )}
...
function SectionLabel({ label, color, dot }: { label: string; color?: string; dot?: boolean }) {
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.xs, marginBottom: spacing.sm, marginTop: spacing.xs }}>
      {dot && <View style={{ width: 7, height: 7, borderRadius: 4, backgroundColor: color ?? colors.gold }} />}
      <Text style={[sl.text, color ? { color } : {}]}>{label}</Text>
    </View>
  );
}
const sl = StyleSheet.create({
  text: { fontSize: fonts.xs, fontWeight: '800', color: colors.textMuted, letterSpacing: 2 },
});

// ── Match card ────────────────────────────────────────────────
function MatchCard({ match, firstName, onWatch }: {
  match: MatchRow;
  ...
  function renderSide(playerIds: string[], team: { name: string } | null, teamId: string | null, label: string) {
    ...
    if (match.is_singles && playerIds.length === 1) {
      ...
      return av
        ? <Image source={av} style={mc.avatar} />
    ...
    return (
      <View style={mc.pairRow}>
        {playerIds.map((id, i) => {
          ...
          return av
            ? <Image key={id} source={av} style={[mc.pairAv, i > 0 && mc.pairOverlap]} />
    ...
    >
      ...
      <View style={mc.teamsRow}>
        ...
        <View style={mc.side}>
          ...
          <Text style={[mc.sideName, aheadSide === 'home' && { color: colors.white }]} numberOfLines={1}>
            {homeLabel}
          </Text>
        </View>
        ...
        <View style={[mc.side, mc.sideRight]}>
          <Text style={[mc.sideName, mc.sideNameRight, aheadSide === 'away' && { color: colors.white }]} numberOfLines={1}>
            {awayLabel}
          </Text>
...
const mc = StyleSheet.create({
  ...
  matchNum: { fontSize: fonts.xs, fontWeight: '700', color: colors.textMuted, letterSpacing: 1 },
  dayTag: { fontSize: fonts.xs, color: colors.textMuted, flex: 1 },
  watchLink: { fontSize: fonts.xs, fontWeight: '700', color: colors.live, letterSpacing: 0.3 },
  ...
  avatarInitial: { fontSize: fonts.sm, fontWeight: '800', color: colors.gold },
  pairRow: { flexDirection: 'row' },
  pairAv:  { width: 26, height: 26, borderRadius: 13, overflow: 'hidden' },
  pairOverlap: { marginLeft: -8 },
  pairInitial: { fontSize: 10, fontWeight: '700', color: colors.gold },
  sideName: { flex: 1, fontSize: fonts.md, fontWeight: '800', color: colors.textSecondary },
  sideNameRight: { textAlign: 'right' },
  centre: { alignItems: 'center', paddingHorizontal: spacing.xs, minWidth: 68 },
  statusLabel: { fontSize: fonts.xl, fontWeight: '900', color: colors.textSecondary, textAlign: 'center', letterSpacing: 0.5 },
  thru: { fontSize: 9, fontWeight: '700', color: colors.textMuted, letterSpacing: 1.5, marginTop: 2 },
  ...
  dotCurrent: { backgroundColor: colors.gold, opacity: 0.5 },
  leaderStrip: {
    paddingLeft: spacing.sm, paddingVertical: 4,
    borderRadius: radius.sm, backgroundColor: 'rgba(255,255,255,0.02)',
  },
  leaderText: { fontSize: fonts.xs, fontWeight: '700', color: colors.textMuted, letterSpacing: 0.5 },
});
