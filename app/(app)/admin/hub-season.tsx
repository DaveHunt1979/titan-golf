...
const TILES = [
  { key: 'create',  label: 'Create Season', sub: 'Set up a new year-long league',      icon: 'add-circle-outline' as const, route: '/(app)/admin/season-create' },
  { key: 'manage',  label: 'Seasons',       sub: 'View and manage existing Seasons',   icon: 'list-outline'       as const, route: '/(app)/admin/season-manage' },
] as const;
    ...
    <View style={[s.container, { backgroundColor: dc.bg }]}>
      ...
      <View style={[s.header, { borderBottomColor: dc.border }]}>
        ...
        <View style={s.headerCenter}>
          ...
          <Text style={[s.headerSub, { color: GREEN }]}>SEASON</Text>
        </View>
        <View style={{ width: 60 }} />
      </View>
        ...
        <View style={s.grid}>
          {TILES.map(t => (
            <TouchableOpacity
              ...
              activeOpacity={0.75}
            >
              <View style={[s.tileIcon, { backgroundColor: `${GREEN}18`, borderColor: `${GREEN}55` }]}>
                <Ionicons name={t.icon} size={24} color={GREEN} />
              </View>
