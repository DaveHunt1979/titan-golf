// The actual Prize Category CRUD, extracted out of app/(app)/admin/prizes.tsx
// so it can be reused as a Tournament Builder step (Rick's brief, 2026-08-22,
// section 4.7: "Use the existing Prize Category functionality where possible.
// Do not create a duplicate prize system.") without duplicating the logic —
// admin/prizes.tsx now just wraps this in its own header/back button, and the
// builder wraps the exact same component in its own step header instead.
...
export default function PrizeCategoriesEditor({ competitionId }: { competitionId: string }) {
  const [compFormat, setCompFormat]   = useState<string | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: comp }, { data: cats }] = await Promise.all([
      supabase.from('competitions').select('format,kronos_overall_prize,handicaps_locked_at').eq('id', competitionId).single(),
    ...
    if (comp) {
      setCompFormat((comp as any).format ?? null);
  ...
  const individualLabel = individualBoardLabel(compFormat);
  ...
  return (
    <View>
      ...
      <View style={s.catCard}>
        <Text style={s.catName}>{individualLabel} Overall Winner</Text>
        <Text style={[s.catRange, { marginBottom: 12 }]}>Prize for the outright {individualLabel} champion — separate from the divisions below.</Text>
