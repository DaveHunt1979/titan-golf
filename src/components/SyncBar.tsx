...
export default function SyncBar({ status }: Props) {
  const { state, pendingCount, lastSyncedAt, syncNow } = status;
  const cfg = CONFIG[state] ?? CONFIG.idle;
  const showSyncBtn = (state === 'offline' || state === 'error') && pendingCount > 0;
