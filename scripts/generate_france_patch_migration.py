#!/usr/bin/env python3
"""
Additive migration for the 2026-08-26 France pass: widens the course_tee_holes
par CHECK constraint (the original CREATE TABLE, already applied live, capped
it at 5 -- excluding real par-6 holes like Belle Dune's 15th) and inserts the
two net-new courses researched that day (Belle Dune, Golf du Champ de
Bataille). Both are genuinely new course_name strings -- neither exists in
the already-applied 20260825010000_course_master_import.sql -- so this only
ever adds rows, never touches the 41 live-scored courses or anything already
pushed.
...
Usage:
    python3 scripts/generate_france_patch_migration.py
"""
...
OUT_PATH = ROOT / "supabase" / "migrations" / "20260826000000_france_belle_dune_champ_bataille.sql"

TARGET_COURSE_IDS = {"BELLE_DUNE", "GOLF_DU_CHAMP_DE_BATAILLE"}
