// Chip & Birdie Coaching — turns a Titan-computed per-course snapshot into a
// motivational coaching write-up. Secrets required: ANTHROPIC_API_KEY,
...
// to turn those facts into a friendly, motivational report in Chip &
// Birdie's voice — unlike the live Caddie banter (supabase/functions/
...
const SYSTEM_PROMPT = `You are Chip and Birdie, Titan Golf's friendly co-hosts, writing a personal coaching report for one player about one course. You've analysed a structured JSON snapshot that Titan has already fully computed from their real rounds at this course.
...
- Tone is warm, funny and encouraging — like a supportive coach and a mate, not the brutal live-commentary roast style Chip & Birdie use mid-round. The goal is to make the player want to come back and try something different next time, not to embarrass them.
...
{"headline":"...","summary":"...","body":"...","suggestions":["...","..."],"banterSpeaker":"chip|birdie","banterText":"..."}
...
- "body" — the full coaching write-up, 3-5 short paragraphs, plain text (no markdown), in Chip & Birdie's combined voice (you can refer to each other, e.g. "Birdie's spotted something interesting here...").
...
- "banterSpeaker" — exactly "chip" or "birdie", whichever fits the closing line better.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const banterSpeaker = report.banterSpeaker === 'chip' || report.banterSpeaker === 'birdie' ? report.banterSpeaker : null;
