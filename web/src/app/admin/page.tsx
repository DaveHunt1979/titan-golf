  ...
  return (
    <div className="mx-auto max-w-screen-xl px-6 py-12">
      ...
      <div className="mb-10">
        ...
        <p className="mt-2 text-neutral-400">Welcome back, {firstName}. Manage your society from here.</p>
      </div>
      ...
      <div className="mb-8 flex items-center justify-between rounded-2xl border border-[#D4AF37]/20 bg-[#111111] px-6 py-4">
        <div>
          <div className="text-xs font-bold uppercase tracking-widest text-neutral-500">Society Join PIN</div>
          <div className="mt-1 text-xs text-neutral-600">New players enter this in the app to join</div>
        </div>
      ...
      <div className="mb-8">
        <div className="mb-4 flex items-center justify-between">
          ...
          <a
            href="/tournament/new"
            className="rounded-lg bg-[#D4AF37] px-4 py-2 text-sm font-bold text-[#000000] transition-opacity hover:opacity-90"
          >
            + New
          </a>
        </div>
        ...
        {(comps ?? []).length === 0 ? (
          <div className="rounded-2xl border border-[#1c1c1c] bg-[#111111] p-10 text-center">
            <div className="text-3xl mb-3">🏆</div>
            <p className="text-neutral-400">No competitions yet. Create one above.</p>
          </div>
        ) : (
          <div className="overflow-hidden rounded-2xl border border-[#1c1c1c]">
            <div className="grid grid-cols-[1fr_6rem_8rem_6rem] gap-4 border-b border-[#1c1c1c] bg-[#111111] px-5 py-3">
              {['Competition', 'Status', 'PIN', 'Created'].map(h => (
                <div key={h} className="text-xs font-bold uppercase tracking-widest text-neutral-500">{h}</div>
              ))}
            </div>
            {(comps ?? []).map((comp: any, i: number) => {
              const statusColor = comp.status === 'active' ? 'text-[#4ade80]' : comp.status === 'complete' ? 'text-neutral-500' : 'text-[#D4AF37]';
              return (
                <div
                  key={comp.id}
                  className={`grid grid-cols-[1fr_6rem_8rem_6rem] gap-4 items-center border-b border-[#1c1c1c] px-5 py-4 last:border-0 ${i % 2 === 0 ? 'bg-[#000000]' : 'bg-[#0a0a0a]'}`}
                >
      ...
      <div>
        ...
        <div className="overflow-hidden rounded-2xl border border-[#1c1c1c]">
          <div className="grid grid-cols-[1fr_8rem_6rem] gap-4 border-b border-[#1c1c1c] bg-[#111111] px-5 py-3">
          ...
          {(players ?? []).slice(0, 30).map((m: any, i: number) => (
            <div
              key={m.player_id}
              className={`grid grid-cols-[1fr_8rem_6rem] gap-4 items-center border-b border-[#1c1c1c] px-5 py-3 last:border-0 ${i % 2 === 0 ? 'bg-[#000000]' : 'bg-[#0a0a0a]'}`}
            >
              <div className="text-sm font-semibold text-white">{m.players?.display_name ?? '—'}</div>
              <div className="text-sm text-neutral-400">{m.players?.handicap_index != null ? m.players.handicap_index.toFixed(1) : '—'}</div>
              <div className={`text-xs font-bold uppercase ${m.role === 'admin' || m.role === 'owner' ? 'text-[#D4AF37]' : 'text-neutral-500'}`}>{m.role}</div>
            </div>
          ))}
        </div>
      </div>
