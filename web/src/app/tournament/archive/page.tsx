    ...
    <div className="mx-auto max-w-screen-xl px-6 py-12">
      ...
      <div className="mb-10 flex items-end justify-between">
        ...
        {isAdmin && (
          <a
            href="/tournament/new"
            className="rounded-xl bg-[#D4AF37] px-5 py-3 text-sm font-800 text-[#000000] transition-opacity hover:opacity-90"
          >
            + New Competition
          </a>
        )}
      ...
      {years.length > 0 && (
        <section className="mb-12">
          <h2 className="mb-4 text-xs font-bold uppercase tracking-widest text-neutral-500">Champions</h2>
          <div className="space-y-4">
            {years.map(year => {
              ...
              return (
                <div key={year} className="overflow-hidden rounded-2xl border border-[#D4AF37]/20 bg-[#111111]">
                    ...
                    {tour && (
                      <div className="flex items-center gap-4 px-6 py-4">
                        ...
                        <div className="flex-1">
                          <div className="text-xs font-bold uppercase tracking-widest text-neutral-500">{tour.award_name}</div>
                          <div className="mt-0.5 text-xl font-black text-white">{tour.winner_name}</div>
                          {tour.detail && <div className="mt-0.5 text-sm text-neutral-400">{tour.detail}</div>}
                        </div>
                      </div>
                    )}
                    {kronos && (
                      <div className="flex items-center gap-4 px-6 py-4">
                        <span className="text-2xl">⚡</span>
                        <div className="flex-1">
                          <div className="text-xs font-bold uppercase tracking-widest text-neutral-500">{kronos.award_name}</div>
                          <div className="mt-0.5 text-xl font-black text-white">{kronos.winner_name}</div>
                          {kronos.detail && <div className="mt-0.5 text-sm text-neutral-400">{kronos.detail}</div>}
                        </div>
                      </div>
                    )}
                    {others.map((ch, i) => (
                      <div key={i} className="flex items-center gap-4 px-6 py-4">
                        <span className="text-2xl">🎖</span>
                        <div className="flex-1">
                          <div className="text-xs font-bold uppercase tracking-widest text-neutral-500">{ch.award_name}</div>
                          <div className="mt-0.5 text-xl font-black text-white">{ch.winner_name}</div>
                          {ch.detail && <div className="mt-0.5 text-sm text-neutral-400">{ch.detail}</div>}
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      {completed.length > 0 && (
        <CompSection title="Completed" comps={completed} isAdmin={isAdmin} pinDisplay={pinDisplay} badge="DONE" badgeColor="text-neutral-500 border-neutral-700" />
      )}
      ...
      {years.length === 0 && (comps ?? []).length === 0 && (
        <div className="rounded-2xl border border-[#1c1c1c] bg-[#111111] p-16 text-center">
          ...
          <p className="mt-2 text-neutral-400">Create your first competition to get started.</p>
          {isAdmin && (
            <a href="/tournament/new" className="mt-6 inline-block rounded-xl bg-[#D4AF37] px-6 py-3 text-sm font-800 text-[#000000]">
              Create Competition
            </a>
          )}
        </div>
...
function CompSection({ title, comps, isAdmin, pinDisplay, badge, badgeColor }: {
  ...
  return (
    <section className="mb-10">
      <h2 className="mb-4 text-xs font-bold uppercase tracking-widest text-neutral-500">{title}</h2>
      <div className="space-y-3">
        {comps.map(comp => (
          <div key={comp.id} className="rounded-xl border border-[#1c1c1c] bg-[#111111] px-6 py-5">
            <div className="flex items-start justify-between gap-4">
              ...
              {isAdmin && (
                <div className="text-right">
                  <div className="text-[10px] font-bold uppercase tracking-widest text-neutral-600">PIN</div>
